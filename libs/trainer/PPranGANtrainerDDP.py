...
class PPransGANtrainer(Base_Trainer):
    ...
    def __init__(self, jsonData, sharedFilePath,restart=False, startEpoch=0, ddp=True):
    ...
    def train(self, gpu, size): 
        ...
        # Initialize DDP Information
        self.gan = PPransGAN(self.jsonData, gpu= f"cuda:{gpu}" if torch.cuda.is_available() else "cpu") 
        print(f"Now Initialize GPU : {gpu} | Number Of GPU : {size}")
        self.initialize(gpu, size)
        ranks = list(range(size))
        group_g = torch.distributed.new_group(ranks=ranks)
        group_d = torch.distributed.new_group(ranks=ranks)
        group_p = torch.distributed.new_group(ranks=ranks)        
        ...
        # when starting with check point
        if self.restart:
            ...
            self.gan.netG, self.gan.netD, self.gan.optimG, self.gan.optimD, self.startEpoch = load(os.path.join(self.gan.ckptDir, self.name), self.gan.netG, self.gan.netD, self.gan.optimG, self.gan.optimD, gpu=now)
            self.startEpoch += 1
            print(f"Load Done.. Epoch : {self.startEpoch}")
        
        # DDP Models
        self.gan.netG = DDP(self.gan.netG.cuda(gpu), device_ids=[gpu], output_device=[gpu], process_group=group_g, broadcast_buffers=True, find_unused_parameters=True)
        self.gan.netD = nn.SyncBatchNorm.convert_sync_batchnorm(self.gan.netD)
        self.gan.netD = DDP(self.gan.netD.cuda(gpu),  device_ids=[gpu], output_device=[gpu], process_group=group_d, broadcast_buffers=False, find_unused_parameters=True)
        self.gan.perceptualNet = DDP(self.gan.perceptualNet.cuda(gpu), device_ids=[gpu], output_device=[gpu], process_group=group_p, broadcast_buffers=True, find_unused_parameters=True)
        ...
        for epoch in range(self.startEpoch, self.gan.epochs):
            ...
            if epoch % 10 == 0:
                ...
                stringLog = f"GPU {gpu} Train {epoch} / {self.gan.epochs} Epochs | LOSS {loss} | PSNR Acc {acc} | G {lossG} | D {lossD}"
            ...
            if epoch % self.gan.saveIter == 0:
                ...
                with torch.no_grad():
                    for index, data in enumerate(self.loaderVal):
                        ...
                        # Save Every First Value Image and GPU 0
                        if index == 0 and gpu == 0:
    ...
    def checkPoint(self, gpu, epoch):
        ...
        dict_model = torch.load(os.path.join(self.gan.ckptDir, self.name, f"{self.name}_{epoch}.pth"), map_location=mapLocation)
        self.gan.netG.module.load_state_dict(dict_model["netG"])
        self.gan.netD.module.load_state_dict(dict_model["netD"])
